...
class BannerController extends Controller
{
    ...
    public function add_banner_store(Request $request)
    {
        ...
        if (!empty($request->file('desktop_banner'))) {
            if (!empty($category->desktop_banner) && file_exists('upload/desktop_banner/' . $category->desktop_banner)) {
                unlink('upload/desktop_banner/' . $category->desktop_banner);
            }

            $file = $request->file('desktop_banner');
            $randomStr = Str::random(30);
            $filename = $randomStr . '.' . $file->getClientOriginalExtension();
            $file->move('upload/desktop_banner/', $filename);
            $category->desktop_banner = $filename;
        }
        if (!empty($request->file('mobile_banner'))) {
            if (!empty($category->mobile_banner) && file_exists('upload/mobile_banner/' . $category->mobile_banner)) {
                unlink('upload/mobile_banner/' . $category->mobile_banner);
            }

            $file = $request->file('mobile_banner');
            $randomStr = Str::random(30);
            $filename = $randomStr . '.' . $file->getClientOriginalExtension();
            $file->move('upload/mobile_banner/', $filename);
            $category->mobile_banner = $filename;
        }
